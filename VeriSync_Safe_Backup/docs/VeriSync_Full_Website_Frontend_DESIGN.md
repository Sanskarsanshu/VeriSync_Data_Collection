# VeriSync Full Website Frontend — DESIGN.md

> **Document status:** Master frontend design specification  
> **Product:** VeriSync  
> **Surfaces covered:** Public Website, Authentication, Admin Portal, Teacher Portal, Student Portal  
> **Primary platforms:** Desktop and tablet web  
> **Student priority platform:** Mobile web  
> **Default theme:** Light  
> **Optional themes:** Dark and System  
> **Primary font:** Inter  
> **Only icon library:** Lucide React  
> **Primary chart library:** ApexCharts  
> **Frontend stack:** Next.js App Router, TypeScript strict mode, Tailwind CSS, shadcn/ui, TanStack Query, TanStack Table, React Hook Form, Zod  
> **Project target:** Fully functional college mini project with a simple HTTPS classroom deployment  
> **Data rule:** Real APIs and PostgreSQL persistence; no permanent frontend mocks  
> **Scope:** Attendance management only

---

# 0. Master Purpose

This file is the single source of truth for the complete VeriSync frontend. It extends the approved Admin design system to the Public Website, shared Authentication, Teacher Portal and Student Portal without changing the established visual direction.

VeriSync must be built as one coherent product across all roles. The Admin experience is desktop-primary and data-dense, the Teacher experience must work well on laptop/tablet/mobile, and the Student experience is mobile-first.

The seven approved phases remain:
...
No later-phase surface may be called complete before its real backend phase is working and the user explicitly confirms progression.

## 0.1 Product Definition

VeriSync is a privacy-preserving multi-method attendance system supporting:

- Institutional and academic setup
- Admin management of Teachers and Students
- Teacher assignments and Subject Offerings
- Timetables and scheduled classes
- Teacher-camera face attendance
- Dynamic QR attendance
- Verified-email OTP attendance
- Attendance Records and Matrix
- Percentages and shortage warnings
- Correction workflows
- Reports, Security and Audit Logs

VeriSync is not an LMS, social network, messaging application, assignment portal or file-sharing platform.

## 0.2 Product Promise

> **Reliable attendance, verified identity, clear accountability.**

## 0.3 Cross-Surface Data Ownership

Student personal information is supplied by the Student during website registration. The frontend must never invent Student names, roll numbers, registration numbers, emails, phone numbers, passwords, consent or biometric captures. Academic choices come from Admin-created options and remain subject to Admin verification.

Every working feature must use:
...
# PART I — VeriSync Shared Design System and Admin Frontend

> **Document status:** Final design specification  
> **Product:** VeriSync  
> **Surface:** Shared Design System and Admin Portal  
...
This file is the single source of truth for the visual design, information architecture, interaction model, responsive behaviour, accessibility requirements, and reusable UI patterns of the VeriSync Admin Frontend.
...
VeriSync must visually feel like a serious SaaS product used by colleges and universities.
...
VeriSync is an attendance-management platform.
...
The Teacher will no longer be able to access VeriSync. Historical
...
```text
Read DESIGN.md before implementing any VeriSync Admin Frontend screen.
...
Every page must look like part of one coherent VeriSync SaaS design system,
...
The final VeriSync Admin Frontend design is:
...
# PART II — Public Website and Shared Authentication

# 37. Public Website Purpose

The public website presents VeriSync as a professional attendance product and provides clear entry points for Admin, Teacher and Student users.

It must use real product screenshots after the role portals exist. It must never use fabricated testimonials, unsupported accuracy claims, fake institution logos or invented usage statistics.

## 37.1 Public Navigation
...
# 38. Landing Page Structure

1. Sticky navigation
2. Hero section
3. Trust and product-context strip
4. Attendance problem/solution section
5. Attendance-method showcase
6. Admin–Teacher–Student workflow
7. Real product screenshots
8. Security and privacy section
9. Analytics and reporting section
10. How VeriSync works
11. FAQ
...
> **Attendance you can verify. Records you can trust.**

Supporting copy:

> VeriSync connects academic setup, Teacher-led attendance and Student identity verification in one secure attendance platform.
...
The hero visual must use an actual product composition or real screenshots. Do not use generic stock photographs.

## 38.2 Attendance Method Showcase

### Teacher-Camera Face Attendance

The Teacher starts a class session, opens the camera and Students approach one by one. The system checks liveness, recognises the Student, validates class eligibility and records attendance.

### Dynamic QR Attendance

The Teacher displays a session-bound, short-lived QR code. Eligible authenticated Students scan it through the Student Portal.

### Verified-Email OTP Attendance

A Student-specific OTP is sent to the Student’s verified registered email as a controlled fallback method.

## 38.3 Role-Based Workflow

```text
Admin configures institution, academic structure and assignments
        ↓
Teacher opens an authorised class and starts Attendance
        ↓
Student is verified using Teacher Camera, QR or Email OTP
        ↓
Attendance Records, Matrix, analytics and reports update
```

## 38.4 Public Security Section

Explain in plain language:

- Student consent is required for face enrolment
- Face embeddings are used for recognition
- Embeddings are never displayed to users
- Raw images, when retained, remain private
- QR tokens expire
- OTPs are hashed, time-limited and single-use
- Duplicate attendance is blocked across all methods
- Sensitive actions are recorded in Audit Logs

Do not publish unsupported face-recognition accuracy percentages.

# 39. Shared Authentication Experience

## 39.1 Role Entry

The login experience must clearly distinguish:

- Admin Login
- Teacher Login
- Student Login

Do not silently infer the portal from an email address.

## 39.2 Shared Authentication States
...
- Restrained split-screen layout
- Form panel remains visually dominant
- Brand panel may contain a subtle product illustration or screenshot
...
## 39.4 Authentication Request Flow
...
# PART III — Teacher Portal

# 40. Teacher Portal Positioning

The Teacher Portal must optimise repeated classroom operations. It must be efficient on a laptop, tablet and phone, and the live Attendance screens must remain legible in a real classroom.

Teacher access is restricted to officially assigned Subjects, Sections, Courses and Students.

# 41. Teacher Navigation
...
2. Is any Attendance Session active?
3. Which class requires Attendance next?
4. Which Students have low attendance?
5. Which correction requests require a recommendation?
...
Institution-controlled fields must be read-only with an explanation. A Teacher must not silently edit Department, Employee ID, designation or institutional assignment data.

# 44. Assigned Subjects and Sections
...
The page must never reveal unassigned academic records.
...
Do not expose Student face embeddings, OTP history or private security fields.

# 46. Start Attendance Wizard

Steps:

1. Select assigned Subject Offering
2. Select Section
3. Select Scheduled Class
4. Select Attendance method
5. Select allowed duration
6. Review class context
7. Start Attendance
...
Teacher-Camera Face Attendance is the primary recommended method.

# 47. Teacher-Camera Face Attendance

This is the most important Teacher screen.

## 47.1 Desktop/Tablet Layout

```text
┌──────────────────────────────┬─────────────────────────────┐
│ Camera preview               │ Session summary             │
│ Face-position guide          │ Eligible / Present / Left   │
│ Lighting/liveness feedback   │ Latest recognition          │
│ Recognition status           │ Recent Students             │
│ Camera controls              │ Failed attempts             │
└──────────────────────────────┴─────────────────────────────┘
```
...
- Camera first
- Session metrics below camera
- Recent recognition in a bottom sheet
- Sticky End Session button
- No tiny multi-column table
...
> Ask one Student to stand in front of the camera.

### No Face

Message:

> Position the Student’s face inside the guide.
...
Recognition must not run when multiple faces are visible.

### Low Quality

Message:

> Improve lighting and keep the camera steady.
...
Display the Student identity and original marked time. Do not create another Attendance Record.

### Wrong Section

Message:

> This Student is not eligible for the selected class.

### No Match

Message:

> Student not recognised. Ask the Student to reposition and try again.
...
Disable camera processing and show the final session summary.

## 47.5 Recognition Behaviour

- Exactly one face at a time
- Process compressed frames periodically
- Apply a cooldown after success
- Do not repeatedly mark a Student who remains in frame
- Use a clear visual confirmation
- Optional short sound confirmation may be enabled
- Never expose raw embeddings
- Never force a low-confidence match
- Borderline matches become Pending Review only when backend policy supports it

# 48. Dynamic QR Attendance

The Teacher QR page must be readable from a classroom projector.
...
Do not display the stored token hash as the QR value.

# 49. Email OTP Attendance Session
...
The Teacher must never see plaintext OTPs.
...
Update through bounded polling, SSE or existing WebSocket support. Do not require a full-page refresh.

# 51. Completed Attendance Session
...
- Submit the recommendation to Admin

The Teacher must never directly overwrite official Attendance Records.
...
- Scheduled versus conducted classes
- Attendance-method distribution

# 54. Teacher Route Inventory
...
/teacher/attendance/session/[id]/face
/teacher/attendance/session/[id]/qr
/teacher/attendance/session/[id]/otp
/teacher/attendance/session/[id]/live
...
# PART IV — Student Portal

# 55. Student Portal Positioning

The Student Portal is the highest-frequency, most mobile-critical surface. Registration, email verification, face enrolment, QR scanning, OTP attendance, history and correction requests must work smoothly on a normal mobile browser.

A Student may access only their own profile, Attendance Records, analytics, alerts and correction requests.
...
# 57. Student Registration Wizard
...
7. Review
8. Submit
9. Await Admin approval

The active step, completed steps and remaining steps must always be visible.

## 57.1 Identity Details
...
- Email address
- Phone number
- Date of birth only when institutionally required
- Optional profile photograph

Do not prefill invented personal values.

## 57.2 Academic Selection

The Student selects from Admin-created options:
...
When a parent field changes, dependent values must clear.

## 57.3 Account Security
...
Do not display or preserve plaintext password after submission.

# 58. Student Email Verification
...
- Resend cooldown
- Attempts remaining
- Change email action before verification
...
The countdown must not be announced by a screen reader every second.

# 59. Biometric Consent

Explain in clear language:

- Why face data is required
- What is stored
- That face embeddings are not shown to users
- Raw-image retention policy
- How face re-enrolment works
- What happens when consent is withdrawn

Require explicit consent before camera access.

# 60. Student Face Enrolment
...
Do not mark enrolment complete until the backend confirms that the enrolment and embedding are persisted against the authenticated `studentId`.

Never show the embedding in the browser.

# 61. Registration Review and Approval
...
The Pending Approval screen shows:
...
The Student account must not become active before Admin approval.
...
The active method is determined by the current Teacher Attendance Session.
...
For Teacher-Camera mode, the Student does not open their own camera. Show:

> Attendance is being marked through the Teacher’s camera.

The page may display waiting, recognised, already recorded or session closed states.

# 64. Student QR Scanner

Use a mobile-first full-screen scanner.
...
# 65. Student Email OTP Attendance

Flow:

1. Open active class
2. Request attendance OTP
3. Show masked verified email
4. Enter OTP
5. Verify
6. Show Attendance recorded

Visible rules:

- OTP is Student-specific
- OTP is tied to the active Attendance Session
- OTP expires
- OTP is single-use
- Resend has a cooldown
- Verified email is required

# 66. Attendance Result
...
- Clear reason
- Safe retry only when permitted
- Support reference ID
- Return to active attendance

Do not show ambiguous raw backend messages.

# 67. Attendance History
...
On mobile, use structured cards. On desktop/tablet, a compact table is acceptable.
...
Holiday and Not Applicable must not reduce the Attendance percentage.
...
- Percentage gap
- Subject breakdown
- Classes needed when provided by backend
- Recent trend
- Relevant correction action

Use clear warning styling without alarming or shaming language.

# 70. Student Correction Request

The Student selects an Attendance Record belonging to their own account.
...
1. Submitted
2. Teacher reviewed
3. Admin approved or rejected
4. Attendance updated

No supporting-file upload is required in the initial attendance-only project.
...
Restricted identity and academic fields cannot be silently changed. The UI must explain when Admin approval is required.

# 72. Face Re-Enrolment

Show:

- Current face-enrolment status
- Reason field
- Request status
- Admin approval status
- New capture flow after approval

The Student must not overwrite active biometric identity without approval.

# 73. Student Route Inventory

```text
/student
/student/dashboard
/student/register
/student/register/verify-email
/student/register/consent
/student/register/face-enrolment
/student/register/review
...
/student/corrections
/student/corrections/new
/student/corrections/[id]
/student/face-enrolment
/student/face-reenrolment
/student/alerts
/student/security
/student/settings
/student/help
...
# PART V — Cross-Role Attendance Rules

# 74. One Authoritative Attendance Result

Teacher Camera, QR and Email OTP must converge on the same Attendance Record model.

A Student cannot mark once through face and again through QR or OTP.

When attendance already exists, show:

> Attendance was already recorded at 10:14 AM through Teacher Camera.

Do not show a generic duplicate error.

# 75. Attendance Method Labels

| Backend | UI label |
|---|---|
| `FACE_CAMERA` | Teacher Camera |
| `QR` | Dynamic QR |
| `EMAIL_OTP` | Email OTP |
| `MANUAL` | Authorised Manual |

# 76. Attendance Status Labels
...
# 77. Session Closure UX

When the Teacher ends a session:

- Teacher marking controls stop
- Student submission controls stop
- Final session counts appear
- Admin live monitoring updates
- Student history refreshes
- Missing eligible Students become Absent only after backend finalisation

# 78. Shared Security and Privacy Rules
...
Use masking where appropriate for:
...
- Embedding arrays
- Biometric vectors
- Download controls for face data
- Public image URLs
- Raw similarity data as the sole decision explanation

# 80. Step-Up Authentication

Require password or MFA confirmation for:

- Changing Admin role/scope
- Resetting face enrolment
- Deactivating accounts
- Undoing approved corrections
- High-scope exports
- Critical Attendance Policy changes
- Session revocation

---

# PART VI — Final Engineering and Quality Contract

# 81. BFF and API Contract
...
- OpenAPI-generated types are preferred
...
# 82. TanStack Query Rules

- Stable query keys
- Include role and academic scope in keys
- Invalidate related analytics after mutations
- Controlled polling for live pages
- Preserve filters and pagination
- Do not refetch the complete app after every write
- Do not use optimistic updates for Attendance decisions, corrections, security actions or authorisation codes

# 83. Honest Empty States

Use copy such as:

- “No Students have registered and been approved yet.”
- “No Teachers have been added.”
- “No active Attendance Sessions.”
- “No correction requests are awaiting review.”
- “No attendance data is available for this period.”
- “No Subjects are assigned to this Teacher.”
- “No active attendance opportunity is available.”
- “Face enrolment has not been completed.”

Do not fill empty screens with invented personal data.

# 84. Performance Contract

- Route-level code splitting
- Lazy-load ApexCharts
- Lazy-load camera and QR libraries
- Server pagination
- Debounced search
...
- Avoid continuous full-resolution video upload
- Load biometric review media only when authorised review opens
- Optimise profile images

# 85. Accessibility Contract
...
- QR error announcements
- Recognition success announcement

# 86. Frontend Phase Completion Gates

## Phase 2 — Admin Frontend

Ready only when:

- Every required Admin page uses real Phase 1 APIs
- Forms persist to PostgreSQL
- Attendance Overview is real
- Matrix is real
- Shortage is backend-calculated
- Corrections persist
- CSV export works
- Audit Logs are real
- Lint, typecheck and build pass without bypasses
...
- Teacher login works
- Assigned scope is enforced
- Start Attendance works
- Teacher-camera face attendance works
- QR works
- OTP session works
- Live count updates
- Session ends and finalises
- History and analytics use real data
- User confirms completion
...
- Face enrolment works
- Admin approval state works
- Student login works
- QR scanning works
- Email OTP attendance works
- Teacher-camera result appears
- History and analytics are real
...
- Public content matches the real product
- Real screenshots are used
- Login and registration links work
- No fabricated claims
- Responsive and accessible
- Demo URL is shareable

# 87. Full Product QA Checklist

## Brand and Design

- [ ] VeriSync name used everywhere
- [ ] Inter used globally
- [ ] Lucide is the only icon library
- [ ] ApexCharts is the only chart library
- [ ] Navy/indigo palette is consistent
- [ ] Light theme is default
- [ ] Dark/System themes work
- [ ] No arbitrary colours or mismatched radii

## Data

- [ ] Real API data
- [ ] No permanent mocks
- [ ] No invented Student data
- [ ] Refresh preserves data
- [ ] Role and scope enforced
- [ ] Loading, empty and error states

## Attendance

- [ ] Teacher-camera one-Student-at-a-time flow
...
- [ ] Session closure
- [ ] Method recorded
- [ ] One Student + one Scheduled Class uniqueness

## Student

- [ ] Student supplies personal data
...
# 88. Final Antigravity Instruction

```text
Read VeriSync_Full_Website_Frontend_DESIGN.md before creating or modifying any
VeriSync frontend screen.

Build one coherent attendance product across the Public Website, Admin Portal,
...
The Admin Portal is desktop-primary and data-dense.
The Teacher Portal must work well on desktop, tablet and mobile.
The Student Portal is mobile-first.

The primary attendance method is Teacher-Camera Face Attendance:
Teacher starts an Attendance Session
→ Teacher opens the camera
→ one Student stands in front of it
→ liveness and recognition run
→ matched studentId resolves the Student name and roll number
→ eligibility and duplicate checks run
→ Attendance is recorded
→ live count updates
→ the next Student approaches

Student personal data is supplied by the Student during website registration.
The Student completes email OTP verification, biometric consent and face enrolment.
Face embeddings are linked through studentId and must never be exposed to the browser.

Dynamic QR and verified-email OTP are secondary/fallback methods. All attendance
methods converge on one authoritative Attendance Record and prevent duplicates
across methods.

Do not build chat, announcements, assignments, materials, social feeds or document
sharing.

Do not use permanent mock data.
Do not invent Student personal data.
Do not expose tokens, OTPs, face embeddings or biometric vectors.
Do not mark a page complete until it uses the real backend and PostgreSQL.

Follow the seven-phase roadmap. Complete one phase, provide evidence, request user
confirmation, and only then begin the next phase.
```

# 89. Final Design Decision

The final VeriSync frontend is:
...
- **Primary action:** Indigo
- **Surfaces:** Soft neutral grey and white
- **Font:** Inter
- **Icons:** Lucide React only
- **Charts:** ApexCharts only
- **Admin:** Desktop-primary and data-dense
- **Teacher:** Responsive and camera-optimised
- **Student:** Mobile-first
- **Primary attendance:** Teacher-camera face recognition
- **Secondary attendance:** Dynamic QR
- **Fallback attendance:** Verified-email OTP
- **Accessibility:** WCAG 2.2 AA
- **Security:** Privacy-aware and audit-conscious
- **Data:** Real APIs and PostgreSQL; no permanent mocks
- **Student data:** Supplied by the Student during registration
- **Delivery:** Fully functional mini project with simple HTTPS classroom deployment
- **Interaction quality:** Calm, fast, explicit and trustworthy
