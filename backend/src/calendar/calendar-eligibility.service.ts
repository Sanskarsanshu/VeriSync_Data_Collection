...
export class CalendarEligibilityService {
  ...
  async checkEligibility(
    ...
    const collegeId =
      course.section.semester.batch.session.programme.department.collegeId;
    ...
    const isSpecialWorkingDay = events.some(
      (e) => e.eventType === 'SPECIAL_WORKING_DAY',
    );
    ...
    const isVacation = events.some(
      (e) => e.eventType === 'VACATION_START' || e.eventType === 'VACATION_END',
    );
    if (isVacation && !isSpecialWorkingDay) {
      return AttendanceEligibility.VACATION;
    }

    const isExam = events.some(
      (e) => e.eventType === 'EXAM_START' || e.eventType === 'EXAM_END',
    );
