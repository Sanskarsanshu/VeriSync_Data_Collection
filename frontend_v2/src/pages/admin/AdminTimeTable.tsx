...
import { Clock, Plus, Filter, FileX } from 'lucide-react';
...
export default function AdminTimeTable() {
  ...
  const [selectedSemester, setSelectedSemester] = useState<string>("3"); // Default to 3 as they just asked for it
      ...
      <div className="max-w-[1400px] mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 pb-12">
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            ...
            <p className="text-muted-foreground mt-1">Map courses and faculty to specific time slots and classrooms.</p>
          </div>
          ...
          <div className="flex gap-3 relative">
            ...
            <Button className="bg-blue-500 hover:bg-blue-600 text-white shadow-lg shadow-blue-500/20 gap-2 rounded-xl h-auto py-2">
              <Plus size={16} /> Add Time Slot
            </Button>

            {isFilterOpen && (
              <div className="absolute top-full mt-2 left-0 md:right-[150px] md:left-auto bg-card border border-border rounded-xl shadow-lg p-4 z-50 min-w-[200px] animate-in fade-in slide-in-from-top-2">
        ...
        {currentData ? (
                      ...
                      {periods.map(periodConfig => {
                        ...
                        return (
                          <td key={periodConfig.id} className="px-3 py-3 border-r relative group">
                            {hasData ? (
                              <div className={`h-full flex flex-col justify-between space-y-2 p-3 rounded-xl bg-background border shadow-sm group-hover:shadow-md transition-all ${cellData.status === 'ADMIN_REVIEW_REQUIRED' ? 'border-red-500/50 bg-red-500/5' : 'border-border group-hover:border-blue-500/30'}`}>
          ...
          <div className="bg-card border border-border rounded-2xl p-16 text-center shadow-sm flex flex-col items-center justify-center mt-6">
            ...
            <p className="text-muted-foreground max-w-md">
              There is currently no timetable data configured for the selected semester. Please select a different semester or create a new schedule.
            </p>
          </div>
        )}
