...
export default function StudentTimeTable() {
  ...
  const getTypeColor = (type: string, status?: string) => {
    ...
    switch(type) {
      ...
      default: return 'bg-violet-500/10 text-violet-700 border-violet-500/20';
    }
  }
                      ...
                      {periods.map(periodConfig => {
                        ...
                        return (
                          <td key={periodConfig.id} className="px-3 py-3 border-r relative group">
                            {hasData ? (
                              <div className={`h-full flex flex-col justify-between space-y-2 p-3 rounded-xl bg-background border shadow-sm group-hover:shadow-md transition-all ${cellData.status === 'ADMIN_REVIEW_REQUIRED' ? 'border-red-500/50 bg-red-500/5' : 'border-border group-hover:border-violet-500/30'}`}>
