...
const DetailView = ({ student, onBack }: { student: Student, onBack: () => void }) => {
  ...
  const allMyCourses = React.useMemo(() => {
    if (!currentTeacher || !currentTeacher.semesterSubjects) return [];
    return Object.values(currentTeacher.semesterSubjects).flat();
  }, [currentTeacher]);

  const viewableCourses = Object.keys(student.matrix).filter(c => allMyCourses.includes(c));

  const [selectedCourse, setSelectedCourse] = useState(viewableCourses[0] || '');
  ...
  React.useEffect(() => {
    if (viewableCourses.length > 0 && !viewableCourses.includes(selectedCourse)) {
      setSelectedCourse(viewableCourses[0]);
    }
  }, [viewableCourses, selectedCourse]);
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700 max-w-7xl mx-auto">
      ...
      <div className="bg-card border border-border rounded-2xl p-6 shadow-sm overflow-hidden flex flex-col">
        <div className="flex justify-between items-start mb-4 flex-wrap gap-3">
          ...
          >
            {viewableCourses.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
        </div>
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-blue-500/10 text-blue-500 text-xs font-semibold mb-3 border border-blue-500/20">
              <Users size={14} /> Records Directory
            </div>
